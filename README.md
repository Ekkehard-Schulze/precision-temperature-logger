...
*Firmware Note:* To generate the binary for this hardware, ensure `#define OPENLOG` is active and `#define ADAFRUIT_FEATHER_LOGGER` is commented out in the C source file.


 Generate the respective binary by uncommenting the openlog "#define OPENLOG" statement in the C source code, and commenting the "#define ADAFRUIT_FEATHER_LOGGER" statement.

 To build the corresponding binary, adjust the C source code configuration variables as follows:
